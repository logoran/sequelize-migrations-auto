...
const { diff, objectSimilar } = require('order-diff');
...
function reverseSequelizeColType(col, prefix = 'Sequelize.') {
    ...
    if (attrObj instanceof Sequelize.NUMBER) {
        let ret = attrObj.key;
        if (options.length) {
            ret += `(${options.length}${(options.decimals) ? `, ${options.decimals}` : ''})`
        }
    ...
    // Sequelize.ARRAY PostgreSQL only.
...
function parseIndex(idx) {
    ...
    if (idx.method) { options.indexType = idx.method; }
    ...
    for (const model in models) {
        ...
        if (models[model].options.indexes.length > 0) {
            ...
            for (const _i in models[model].options.indexes) {
                ...
                idx_out[`${index.name}`] = index;
                delete index.hash;
                delete index.name;
                if (Object.keys(index.options).length === 0) {
...
function parseDifference(previousState, currentState) {
    ...
    const difference = diff(previousState, currentState,
        function (path, key, type) {
            // don't care for model name change.
            if (path.length === 1 && key === 'modelName') {
                return true;
            }
        },
        null,
        function (path) {
            if (path.length === 2 && path[1] === 'schema') return false;
        },
        function (path, lhs, rhs, count) {
            if (path.length === 1 && lhs) {
                // if table is similar modify table name.
                return objectSimilar(lhs.schema, rhs.schema, [], function (path, lhs, rhs, count) {
                    var l = Object.keys(lhs).length;
                    if (l <= 3) {
                        return -1;
                    } else if (l <= 5) {
                        return -2;
            ...
            } else if (path.length === 2 && path[1] === 'indexes') {
                // indexes must equal.
                return true;
            } else if (path.length === 3) {
                if (path[1] === 'schema') {
                    if (lhs) {
                        // column type is the same think as a modify not a new.
                        if (lhs.seqType === rhs.seqType) {
                            return true;
                        } else {
                            return false;
                        }
                    } else {
                        // column move as a complete.
                        return false;
                    }
                } else if (path[1] === 'indexes') {
                    // index move as a complete.
                    return false;
                }
            }
        });
                    ...
                    if (df.path[1] === 'schema') {
                        ...
                        if (df.path.length === 3) {
                            ...
                            actions.push({
                                ...
                                options: _.extend(df.rhs, {after: df.lhs || ''}),
                                depends,
                            });
                            break;
                        }
            ...
            case 'M':
                {
                    ...
                    actions.push({
                        ...
                        tableName,
                        attributeName: df.path[2],
                        options: _.extend(options, {after: df.rhs || ''}),
                        depends,
                    });
                }
                break;
...
function getMigration(actions) {
    const propertyToStr = (obj) => {
        ...
        if (obj.after !== undefined) {
            if (obj.after !== '') {
                vals.push(`"after": "${obj.after}"`);
            } else {
                vals.push(`"first": true`);
            }
        }
    ...
    ${JSON.stringify(action.options)}
                    ...
                    const nameOrAttrs = (action.options && action.indexName && action.indexName != '') ?
                        `"${action.indexName}"` : JSON.stringify(action.fields);
            ...
            case 'removeIndex':
                {
                    const nameOrAttrs = (action.indexName && action.indexName != '') ?
                        `"${action.indexName}"` : JSON.stringify(action.fields);
    ...
    ${nameOrAttrs}
            ...
            case 'renameTable':
                {
                    const res = `{ fn: "renameTable", params: [
    "${action.tableName}",
    "${action.newName}"
] }`;
                    commandsUp.push(res);
                    consoleOut.push(`renameTable table "${action.tableName}" to "${action.newName}"`);
                }
                break;

            case 'renameColumn':
                {
                    const res = `{ fn: "renameColumn", params: [
    "${action.tableName}",
    "${action.attributeName}",
    "${action.newName}"
] }`;
                    commandsUp.push(res);
                    consoleOut.push(`renameColumn table "${action.tableName}" colunm "${action.attributeName}" to "${action.newName}"`);
                }
                break;
...
function duplexMigration(previousModels, currentModels) {
    var previousState = reverseModels(previousModels.sequelize, previousModels.sequelize.models);
    var currentState = reverseModels(currentModels.sequelize, currentModels.sequelize.models);
    let actions = parseDifference(previousState, currentState);
    let downActions = parseDifference(currentState, previousState);
    let migration = getMigration(actions);
    let down = getMigration(downActions);
    migration.commandsDown = down.commandsUp;
    return migration;
}
...
function writeMigration(revision, migration, migrationsDir, name = '', comment = '') {
    ...
    var commandsDown;
    if (migration.commandsDown) {
        commandsDown = `var rollbackCommands = [ \n${migration.commandsDown.join(', \n')} \n];\n`;
...
function executeMigration(queryInterface, filename, pos, cb) {
