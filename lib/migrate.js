    ...
    for (const model in models) {
        ...
        if (models[model].options.indexes.length > 0) {
            ...
            for (const _i in models[model].options.indexes) {
                ...
                delete index.hash;
                delete index.name;
                console.log(Object.keys(index.options), index.options);
        ...
        tables[model].indexes = models[model].options.indexes;
    }
...
function parseDifference(previousState, currentState) {
    ...
    const actions = [];
    const difference = diff(previousState, currentState, null, null,
        function (path) {
            ...
            case 'R':
                {
                    ...
                    if (df.path.length === 3) {
                        if (df.path[1] === 'schema') {
                            ...
                            const options = currentState[tableName].schema[df.rhs];
                            if (options.references) { depends.push(options.references.nodel); }
...
function getMigration(actions) {
    ...
    "${action.newName}"
                    ...
                    commandsUp.push(res);
                    consoleOut.push(`renameTable table "${action.newName}" to "${action.tableName}"`);
                }
                break;
